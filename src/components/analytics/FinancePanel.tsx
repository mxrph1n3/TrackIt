...
import { formatFinanceTip, tFinanceCategory } from '../../i18n/helpers';
...
function mapExpenseSlices(
  ...
  const slices: ExpenseSlice[] = top.map((item) => ({
    id: item.id,
    label: labelFor(item.id, item.name),
...
export function FinanceDonutChart() {
  ...
  const slices = mapExpenseSlices(
    data.expenseCategories,
    (id, fallback) => tFinanceCategory(t, id, fallback),
    t('analytics.other'),
  );
  const cx = SIZE / 2;
  const cy = SIZE / 2;
    ...
    <GlassPanel borderRadius={24} style={{ marginBottom: 16 }}>
      <View className="items-center p-5">
        <Text className="mb-4 self-start text-[10px] font-bold uppercase tracking-widest text-ethereal-slate">
          {t('analytics.monthlyExpenditures')}
        </Text>
        ...
        {isLoading && data.expenseCategories.length === 0 ? (
          ...
          <View className="items-center py-12">
            <Text className="text-sm text-ethereal-slate">{t('analytics.noExpenses')}</Text>
          </View>
        ) : (
          <>
...
export function FinanceInsightCard() {
  ...
  const tip = data.financeTips[0];
  const insight = tip
    ? formatFinanceTip(t, tip)
    : data.expenseCategories.length === 0
      ? t('analytics.smartTipEmpty')
      : t('analytics.smartTipBalanced');

  return (
    <GlassPanel borderRadius={24}>
      <View className="p-5">
        <Text className="text-[10px] font-bold uppercase tracking-widest text-obsidian-primary">
          {t('analytics.smartTip')}
        </Text>
        <Text className="mt-1 text-[10px] text-ethereal-slate/70">{t('analytics.smartTipSub')}</Text>
