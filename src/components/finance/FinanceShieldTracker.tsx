...
export function FinanceShieldTracker({ overview }: FinanceShieldTrackerProps) {
  ...
  const statusLabel = t(`finance.shieldStatus.${overview.shieldStatus}`);
        ...
        {overview.hasBudget ? (
          <>
            ...
            <View className="mt-2 flex-row items-center justify-between">
              ...
              <Text className="text-xs font-semibold" style={{ color: theme.textSecondary }}>
                {t('finance.monthlyLimit')}: {formatMoney(overview.monthlyBudget, overview.displayCurrency)}
              </Text>
            </View>
            <Text className="mt-2 text-[11px] font-semibold" style={{ color: theme.textMuted }}>
              {t('finance.shieldStrength', { percent: overview.shieldStrengthPercent })}
              {overview.isVulnerable ? t('finance.vulnerableDebuff') : ''}
            </Text>
          </>
        ) : (
          <Text className="text-sm leading-5" style={{ color: theme.textSecondary }}>
            {t('finance.activateShieldHint')}
          </Text>
        )}
      </View>
    </GlassPanel>
